...
const { triggerOnChange, assignees, document, scripts, error } = useDocument('CRM Lead', props.leadId)
const doc = computed(() => document.doc || {})

watch(error, (err) => {
  if (err) {
    errorTitle.value = __(err.exc_type == 'DoesNotExistError' ? 'Document not found' : 'Error occurred')
...
const statuses = computed(() => {
  let customStatuses = document.statuses?.length ? document.statuses : document._statuses || []
...
const tabs = computed(() => {
  let tabOptions = [
    ...
    {
      // نُبقي name = 'Events' لعدم كسر أي سلوك داخلي، ونغيّر المعروض
      name: 'Events',
      label: __('Comments'),
      icon: NoteIcon,
    },
...
function openEmailBox() {
  ...
  if (!['Emails', 'Activity'].includes(currentTab.name)) {
...
function reloadAssignees(data) {
  if (data?.hasOwnProperty('lead_owner')) assignees.reload()
}

/** Patch visible texts related to Events → Comments without تعديل مكوّنات داخلية */
function patchEventTexts() {
  const root = window.document
  const nodes = root.querySelectorAll('button, a, span, div, h1, h2, h3, p')
  nodes.forEach((el) => {
    const raw = (el.textContent || '').trim()
    const lower = raw.toLowerCase()
    // أزرار/عناوين عامة
    if (lower === 'schedule an event') {
      el.textContent = 'Create comment'
      return
    }
    if (raw === 'Events') {
    ...
    if (raw === 'No Events Scheduled') {
      el.textContent = 'No Comments'
      return
    }
    // عبارات الـ timeline
    if (raw === 'has created an event') {
      el.textContent = 'has created comment'
      return
    }
    if (raw === 'has updated an event') {
      el.textContent = 'has updated comment'
      return
    }
    if (raw === 'has deleted an event') {
      el.textContent = 'has deleted comment'
      return
    }
  })
}
...
onMounted(() => {
  nextTick(() => {
    patchEventTexts()
    const mo = new MutationObserver(() => patchEventTexts())
    mo.observe(window.document.body, { childList: true, subtree: true, characterData: true })
  })
})
</script>
